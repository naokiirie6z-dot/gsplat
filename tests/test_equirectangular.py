...
class TestEquirectangularMath:
    ...
    def test_round_trip(self):
        ...
        # (margin_factor=0 boundary case, see test_seam_and_pole_boundary
        # below); exclude those from the round-trip check.
    ...
    def test_seam_regression_via_ut_covariance(self):
        ...
        # even negative/garbage, since the "unwrapped" mean can land far
        # outside the image and get culled to radii=0); with the fix it
        # should be valid and small, matching its actual small angular size.
        rx = radii[0, 0, 0].item()
...
def test_rasterization_requires_global_z_order_false():
    """camera_model="equirectangular" with the default global_z_order=True
    must be rejected (see rendering.py's assert) rather than silently
    discarding Gaussians behind the camera.
    """
    from gsplat.rendering import rasterization

    means = torch.zeros(1, 3, device=device)
    quats = torch.tensor([[1.0, 0.0, 0.0, 0.0]], device=device)
    scales = torch.ones(1, 3, device=device) * 0.1
